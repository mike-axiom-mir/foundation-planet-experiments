...
export const
  VERIFIER_ROUTE_OUT_OF_BAND_AUTHORITY_DESIGNATION_DECISION_HAND_PROVIDER_VERIFICATION_RECIPIENT_ROUTE_RESOLUTION_PREFLIGHT_EVALUATE_CAPABILITY_ID =
...
createLandMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRecipientRouteResolutionPreflightContractReceipt(
  ...
  if (!custodyValid(custody)) {
    throw new Error('R140 recipient-route contract needs the exact R139 custody');
  }
...
createLandMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRecipientRouteResolutionPreflight(
  ...
  if (!landMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRecipientRouteResolutionPreflightContractReceiptValid(
      contract, custody) ||
      !declarationsValid(declarations, custody.r139Batch.packets.length)) {
    throw new Error('R140 recipient-route preflight needs the exact R139 custody and bounded declarations');
  }
  ...
  if (new TextEncoder().encode(JSON.stringify(preflight)).length >
    ...
    throw new Error('R140 recipient-route preflight exceeds its resource ceiling');
  }
  return preflight;
}
...
landMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRecipientRouteResolutionPreflightValid(
  ...
  return landMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRecipientRouteResolutionPreflightContractReceiptValid(
    contract, custody) &&
    declarationsValid(declarations, custody.r139Batch.packets.length) &&
...
matrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRecipientRouteResolutionPreflightDescription() {
  return {
    ...
    capabilityId:
      VERIFIER_ROUTE_OUT_OF_BAND_AUTHORITY_DESIGNATION_DECISION_HAND_PROVIDER_VERIFICATION_RECIPIENT_ROUTE_RESOLUTION_PREFLIGHT_EVALUATE_CAPABILITY_ID,
    statement:
      'R140 exact-binds future R139 decision-hand provider-verification requests and evaluates bounded caller-supplied recipient-route declarations without trusting route providers, resolving an endpoint or recipient, making an authority decision, designating a route, authorizing contact, or transporting a request.',
    boundaries: [
      'The current real R139 request batch is empty, so the current recipient-route preflight is empty.',
      ...
      'No provider identity, independence, implementation, availability, endpoint, recipient, authority decision, route designation, contact, transport, receipt, provider verification, evidence admission, owner/debit closure, persistence, promotion, canonization, or world mutation is produced.'
    ]
  };
}
