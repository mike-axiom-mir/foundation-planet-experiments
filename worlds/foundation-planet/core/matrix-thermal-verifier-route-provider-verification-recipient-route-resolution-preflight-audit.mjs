...
const AUDIT_SCHEMA =
  'axm.foundation-planet.land-matrix-thermal-verifier-route-provider-verification-recipient-route-resolution-preflight-audit/v1';
...
export function auditLandMatrixThermalVerifierRouteProviderVerificationRecipientRouteResolutionPreflight(
  contract, preflight, custody, declarations = []) {
  const findings = [];
  if (!custodyValid(custody)) {
    findings.push('R133_CUSTODY_INVALID_OR_SUBSTITUTED');
  } else {
    const reconstructedContract = expectedContract(custody);
    if (!exact(contract, reconstructedContract)) {
      findings.push('R134_CONTRACT_NOT_EXACTLY_RECONSTRUCTED');
    }
    if (!declarationsValid(declarations, custody.r133Batch.packets.length)) {
    ...
    if (findings.length === 0 &&
      ...
      findings.push('R134_PREFLIGHT_NOT_EXACTLY_RECONSTRUCTED');
    }
  }
  ...
  return withDigest({
    ...
    summary: {
      sourceRequestPacketCount: custody?.r133Batch?.packets?.length || 0,
