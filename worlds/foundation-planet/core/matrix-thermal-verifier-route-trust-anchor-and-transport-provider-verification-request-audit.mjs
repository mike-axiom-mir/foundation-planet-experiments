...
const AUDIT_SCHEMA =
  'axm.foundation-planet.land-matrix-thermal-verifier-route-trust-anchor-and-transport-provider-verification-request-audit/v1';
...
auditLandMatrixThermalVerifierRouteTrustAnchorAndTransportProviderVerificationRequest(
  ...
  if (!exactCustody) {
    findings.push('R132_CUSTODY_INVALID_OR_SUBSTITUTED');
  }
  let eligibleCount = 0;
  if (exactCustody) {
    eligibleCount = eligibleCandidates(custody).length;
    if (!exact(contract, expectedContract(custody))) {
      findings.push('R133_CONTRACT_NOT_EXACTLY_RECONSTRUCTED');
    }
    ...
    if (findings.length === 0 &&
      ...
      findings.push('R133_REQUEST_BATCH_NOT_EXACTLY_RECONSTRUCTED');
    }
  }
  ...
  return withDigest({
    ...
    summary: {
      ...
      selectedInstalledAvailableOrExecutedProviderCount: 0,
      transmittedRequestCount: 0,
      admittedEvidenceCount: 0
    },
    truth: {
      auditCallsR133BuilderOrValidator: false,
      requestDoesNotVerifyOrAuthorizeProvider: true,
      requestDoesNotResolveOrTrustRecipientRoute: true,
      requestDoesNotTransmit: true,
      noProviderSelectionInstallationAvailabilityOrExecution: true,
