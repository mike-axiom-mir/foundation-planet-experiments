...
const AUDIT_SCHEMA =
  'axm.foundation-planet.land-matrix-thermal-verifier-route-out-of-band-authority-designation-decision-hand-provider-binding-preflight-audit/v1';
...
function contractValid(contract, boundary) {
  return boundaryValid(boundary) &&
    digestValid(contract,
      LAND_MATRIX_THERMAL_VERIFIER_ROUTE_OUT_OF_BAND_AUTHORITY_DESIGNATION_DECISION_HAND_PROVIDER_BINDING_PREFLIGHT_CONTRACT_RECEIPT_SCHEMA) &&
    exact(contract, expectedContract(boundary));
}

function preflightValid(preflight, contract, boundary, declarations) {
  return contractValid(contract, boundary) && declarationsBounded(declarations) &&
    digestValid(preflight,
      LAND_MATRIX_THERMAL_VERIFIER_ROUTE_OUT_OF_BAND_AUTHORITY_DESIGNATION_DECISION_HAND_PROVIDER_BINDING_PREFLIGHT_SCHEMA) &&
...
auditLandMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderBindingPreflight(
  ...
  const audit = {
    ...
    sourcePreflight: preflight?.schema ===
      LAND_MATRIX_THERMAL_VERIFIER_ROUTE_OUT_OF_BAND_AUTHORITY_DESIGNATION_DECISION_HAND_PROVIDER_BINDING_PREFLIGHT_SCHEMA
      ? sourceRef(preflight) : null,
    verdicts: {
      exactR137BoundaryReconstructed: contractReconstructed,
      contractReconstructed,
      declarationsAndDigestsReconstructed: preflightReconstructed,
      assessmentAndCandidateReconstructed: preflightReconstructed,
      schemaPermissionControlBudgetRecoveryAndVerificationExact:
        preflightReconstructed,
      ambiguityPreserved: preflightReconstructed,
      selectionExecutionAuthorityDecisionDesignationAndTransportOverclaimsAbsent:
        preflightReconstructed
    },
    summary: {
