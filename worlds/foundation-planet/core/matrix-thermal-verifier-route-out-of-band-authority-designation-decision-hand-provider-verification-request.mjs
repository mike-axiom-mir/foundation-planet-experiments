...
createLandMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRequestContractReceipt(
  ...
  if (!custodyValid(custody)) {
    throw new Error('R139 verification-request contract needs the exact R138 custody');
  }
...
createLandMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRequestBatch(
  ...
  const eligibleCount = exactCustody
    ? eligibleCandidatesFromValidCustody(custody).length : 0;
  if (!exactCustody || !exact(contract, expectedContract(custody)) ||
      !requestOptionsValid(options, eligibleCount)) {
    throw new Error('R139 request batch needs the exact R138 custody and a bounded request window whenever an eligible candidate exists');
  }
  ...
  if (new TextEncoder().encode(JSON.stringify(batch)).length >
    ...
    throw new Error('R139 decision-hand verification-request batch exceeds its resource ceiling');
  }
  return batch;
}
...
landMatrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRequestBatchValid(
  ...
  if (!custodyValid(custody)) return false;
  return exact(contract, expectedContract(custody)) &&
...
matrixThermalVerifierRouteOutOfBandAuthorityDesignationDecisionHandProviderVerificationRequestDescription() {
  return {
    ...
    capabilityId:
      VERIFIER_ROUTE_OUT_OF_BAND_AUTHORITY_DESIGNATION_DECISION_HAND_PROVIDER_VERIFICATION_REQUEST_CREATE_CAPABILITY_ID,
    statement:
      'R139 exact-binds an R138 compatible-unverified decision-hand candidate into one bounded independent verification request without resolving a recipient route, transmitting, trusting, selecting, installing, executing, authenticating an authority seat, deciding, or designating a route.',
    boundaries: [
      'The current real R138 candidate inventory is empty, so the current request batch is empty and no request metadata is invented.',
      'A synthetic compatible candidate receives eight blocking proof requirements; its claimed independent verifier remains caller-supplied and the verifier endpoint, identity, authority, and transport route remain unresolved.',
      'No request verifies a provider, native decision receipt schema, authority seat, signature, key, availability, criterion, evidence item, endpoint, recipient, transport, decision, designation, source owner, debit, persistence, promotion, canonization, or world mutation.'
    ]
  };
}
