...
} from './matrix-thermal-historical-source-host-governance-trust-root-registry-configuration-response-signer-key-binding-authority-decision-integrity.mjs?v=0.114.0-r114.1';
...
function result(status, detail) {
  return {
    id: 'land-matrix-thermal-historical-source-host-governance-trust-root-registry-configuration-response-signer-key-binding-authority-decision-integrity-contract',
    required: true,
    status,
    statement: 'Exact R113 response-signer-key binding requests gain detached decision and revocation signature-integrity checks under an explicitly untrusted caller policy, while policy delegation, actual binding, registry configuration, authority, persistence, and mutation remain unresolved.',
    detail
  };
}

export function
auditLandMatrixThermalHistoricalSourceHostGovernanceTrustRootRegistryConfigurationResponseSignerKeyBindingAuthorityDecisionIntegrityContract(
  receipt, source) {
  if (!receipt || !source) {
    return result('FAIL', {
      reason: 'the transient R114 projection and its exact R113 source are required'
    });
  }
  const sourceIntegrity =
    landMatrixThermalHistoricalSourceHostGovernanceTrustRootRegistryConfigurationResponseSignerKeyBindingRequestContractReceiptValid(
      source) && exact(receipt.source, {
...
export function
auditLandMatrixThermalHistoricalSourceHostGovernanceTrustRootRegistryConfigurationResponseSignerKeyBindingAuthorityDecisionIntegrityAssessment(
  ...
  return {
    id: 'land-matrix-thermal-historical-source-host-governance-trust-root-registry-configuration-response-signer-key-binding-authority-decision-integrity-assessment',
    required: true,
    status: valid ? 'PASS' : 'FAIL',
    statement: 'The transient decision-integrity assessment must remain bound to exact R114/R113 caller inputs and cannot claim delegated policy authority, signer-key binding, registry configuration, admission, persistence, or mutation.',
    detail: {
      sourcesValid,
      refsExact,
