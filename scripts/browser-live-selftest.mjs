...
async function startServer() {
  const child = spawn(
    process.execPath,
    ['scripts/serve.mjs', '--host', '127.0.0.1', '--port', '0'],
    {
      cwd: repositoryRoot,
...
try {
  ...
  if (!response || response.status() >= 400) {
    fail('Planet entry did not return a successful document response.', {
  ...
  await page.waitForFunction(
    () => document.body.dataset.api?.startsWith('AXMFoundationPlanet/'),
    null,
  ...
  if ((await life.getAttribute('aria-pressed')) !== 'true') {
    fail('Expected Life to be enabled before the reversible interaction.');
  }
  await life.click();
  await page.waitForFunction(
    () => document.getElementById('lifeMaster')?.getAttribute('aria-pressed') === 'false',
    null,
    { timeout: 10_000 },
  );
  ...
  await life.click();
  await page.waitForFunction(
    () => document.getElementById('lifeMaster')?.getAttribute('aria-pressed') === 'true',
    null,
    { timeout: 10_000 },
  );
  ...
  if (pageErrors.length > 0 || fatalResponses.length > 0) {
    fail('Live browser completed interaction with runtime/module errors.', {
      pageErrors,
      fatalResponses,
    });
  }
